        ...
        protected override string ConstructFullInsertStatementInternal()
        {
            ...
            if (this.RefreshOnInsertProperties.Length > 0)
            {
                ...
                else
                {
                    ...
                    {
                        sql += this.ResolveWithCultureInvariantFormatter(
                            $"SELECT {this.ConstructRefreshOnInsertColumnSelection()} FROM {this.GetTableName()} WHERE {this.GetColumnName(this.InsertKeyDatabaseGeneratedProperties[0], null, false)} = LAST_INSERT_ID()");
                    }
                }
            }
